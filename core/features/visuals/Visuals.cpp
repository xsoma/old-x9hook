...
void visuals::draw_name(player_t* entity, box bbox)
{
	...
	if (info.fakeplayer)
		render::text(bbox.x + (bbox.w / 2), bbox.y - 15, render::fonts::esp_font, std::string(info.name) /*+ " (bot)"*/, true, color(255, 255, 255, 255));
	else
...
void visuals::draw_hp(player_t* entity, box bbox)
{
	...
	if (hp < 100)
		render::text(bbox.x - 9, bbox.y + bbox.h - bar - 10, render::fonts::esp_font_small, 0 /*std::to_string(entity->health())*/, false, color(255, 255, 255));
}
...
void visuals::viewmodel_modulate(const model_render_info_t& info)  {
