...
void chams::run(hooks::draw_model_execute::fn ofunc, i_mat_render_context* ctx, const draw_model_state_t& state, const model_render_info_t& info, matrix_t* matrix) {
	...
	static i_material* mat_gloss = interfaces::material_system->find_material("models/player/ct_fbi/ct_fbi_glass", TEXTURE_GROUP_MODEL); // platinum
	static i_material* mat_desync = interfaces::material_system->find_material("models/inventory_items/dogtags/dogtags_outline", TEXTURE_GROUP_MODEL); // blinking
    static i_material* mat_plastic = interfaces::material_system->find_material("simple_regular_reflective", TEXTURE_GROUP_MODEL); // plastic
	static i_material* mat_gold = interfaces::material_system->find_material("models/inventory_items/trophy_majors/gold", TEXTURE_GROUP_MODEL); // gold
	...
	static i_material* mat_glass2 = interfaces::material_system->find_material("models/gibs/glass/glass", nullptr); // dark chrome
	static i_material* mat_glow2 = interfaces::material_system->find_material("vgui/achievements/glow", nullptr); // dark chrome
	static i_material* mat_bubblegum = interfaces::material_system->find_material("models/inventory_items/cologne_prediction/crystal_blue", nullptr); // maybe bubble gum idc
	static i_material* mat_tv = interfaces::material_system->find_material("models/props_interiors/tvebtest", nullptr); // SMPTE
	static i_material* mat_cw = interfaces::material_system->find_material("models/effects/cube_white", nullptr); // SOMETHING CUBE WHITE

	i_material* material = mat_textured;
	i_material* material_arms = mat_normal;
	i_material* material_sleeve = mat_normal;

	switch (variables::chams_arms_mat) {
	case 0:
		material_arms = mat_normal; // textured
		break;
	case 1:
		material_arms = mat_flat; // flat
		break;
	case 2:
		material_arms = mat_desync; // blinking
		break;
	case 3:
		material_arms = mat_crystal; // crystal
		break;
	//case 5:
	//	material = mat_tv; // dark chrome
	//	break;
		/*case 6:
		material = mat_glass; // glass
		break;
	case 7:
		material = mat_glow; // glow
		break;
	case 8:
		material = mat_gold; // gold
		break;*/
	...
	if (variables::chams_material == 0) {
		material = mat_textured; // textured
	}
	else if (variables::chams_material == 1) {
		material = mat_flat; // flat
	}
	else if (variables::chams_material == 2) {
		material = mat_desync; // blinking
	}
	else if (variables::chams_material == 3) {
		material = mat_crystal; // crystal
	}
	else if (variables::chams_material == 4) {
		material = mat_glow2; //  dark chrome
	}
	else if (variables::chams_material == 5) {
		material = mat_glass; // glass
	}
	else if (variables::chams_material == 6) {
		material = mat_glow; // glow
	}
	else if (variables::chams_material == 7) {
		material = mat_gold; // gold
	}
				...
				if (csgo::local_player && player && player->is_player() && !player->dormant() && player->is_alive()) {
					if (player->team() != csgo::local_player->team()) {
						...
						if (variables::backtrack && variables::backtrack_chams && backtracking::records[player->index()].size() > 0 && backtracking::records[player->index()].back().bone_matrix && backtracking::records[player->index()].back().bone_matrix_built) {
							...
							interfaces::render_view->set_blend(float(100 - variables::bttransparency) / 100.f);
						...
						if (variables::antiaim)
						 {
							...
							//	interfaces::render_view->modulate_color(variables::desyncclr2);
							...
							//	interfaces::render_view->modulate_color(variables::desyncclr1);
							//	modulate_shit_clr(mat_eso_glow, variables::desyncclr1);
								interfaces::render_view->set_blend(1.f);
