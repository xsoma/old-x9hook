...
void zgui::checkbox(const char *id, bool &value) {
    ...
    color box = { 36, 61, 36, 255 }; // checkbox filled color
    ...
    if (utils::input::mouse_in_region(draw_pos.x, draw_pos.y, control_width + text_width + 4, control_height)) {
    
        if (value)
            box.a = 90;
        else
            window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x + 1, draw_pos.y + 1}, zgui_render_type::zgui_filled_rect, { 36, 61, 36, 90 }, "X", {control_width - 2, control_height - 2} });
    }
    ...
    window_ctx.render.emplace_back(zgui_control_render_t{{draw_pos.x + 1, draw_pos.y + 1}, zgui_render_type::zgui_filled_rect, value ? box : global_colors.control_idle, "", {control_width - 2, control_height - 2}});
...
void zgui::checkboxwk(const char* id, bool& value, const char* id2, int& value2) {
...
void zgui::combobox(const char *id, std::vector<std::string> items, int &value, const int size) {
    ...
    int control_width = 180; // size
    const int control_height = 15; // 10
	...
	window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x - 90 + control_width + 4, draw_pos.y + 1 }, zgui_render_type::zgui_text, global_colors.color_text, id_split[0], vec2{0, 0}, font });
...
void zgui::multi_combobox(const char *id, std::vector<multi_select_item> items) {
    ...
    const int control_height = 15; // was 11
...
// ========================================================================
void zgui::colorpicker(const char* id, float* value)
{
    std::vector<std::string> id_split = utils::hash::split_str(id, '#');

    const int control_width = 180;
    const int control_height = 14.5;
    int push_height = control_height;
    ...
    window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x + control_width - 90, draw_pos.y + 1 }, zgui_render_type::zgui_text, global_colors.color_text, id_split[0], vec2{0, 0}, font });
    ...
    utils::misc::push_font(render::fonts::watermark_font);


    if (const bool hovered = utils::input::mouse_in_region(draw_pos.x, draw_pos.y, control_width + 4, 60); !hovered && utils::input::key_pressed(VK_LBUTTON)) {
        if (window_ctx.blocking == utils::hash::hash(id))
            window_ctx.blocking = 0;
    }
    if (const bool hovered = utils::input::mouse_in_region(draw_pos.x, draw_pos.y, control_width, control_height); hovered && utils::input::key_pressed(VK_LBUTTON)) {
...
void zgui::colorpicker_zgui(const char* id, float* value)
{
    std::vector<std::string> id_split = utils::hash::split_str(id, '#');
...
void zgui::begin_groupbox(std::string_view title, const vec2 size) {
    ...
    functions.draw_filled_rect(draw_pos.x, draw_pos.y, size.x, size.y, global_colors.color_groupbox_outline);
    functions.draw_filled_rect(draw_pos.x + 1, draw_pos.y + 1, size.x - 2, size.y - 2, global_colors.color_groupbox_bg);

    functions.draw_filled_rect(draw_pos.x + 1, draw_pos.y + 12, size.x - 2, 1, global_colors.color_groupbox_outline);

    if (!title.empty()) {
        functions.draw_text(draw_pos.x + size.x / 2 - text_width / 2, draw_pos.y, global_colors.color_text, font, false, title.data());
    }


    window_ctx.next_cursor_pos = vec2{ cursor_pos.x, cursor_pos.y + size.y + 4 };

    utils::misc::push_cursor_pos(vec2{ cursor_pos.x + 5, cursor_pos.y + 17 });

    utils::misc::push_font(font);
}

void zgui::begin_titlebox(std::string_view title, const vec2 size) {
    const unsigned long font = utils::misc::pop_font();
 ...
 //   functions.draw_filled_rect(draw_pos.x, draw_pos.y, size.x, size.y, global_colors.color_groupbox_outline);
    functions.draw_filled_rect(draw_pos.x + 1, draw_pos.y + 1, size.x - 2, size.y - 2, global_colors.window_background);

 //   functions.draw_filled_rect(draw_pos.x + 1, draw_pos.y + 12, size.x - 2, 1, global_colors.color_groupbox_outline);

    if (!title.empty()) {
        functions.draw_text(draw_pos.x + size.x / 2 - text_width / 2, draw_pos.y + -3, global_colors.color_text, font, false, title.data());
    }
...
void zgui::begin_groupbox2(std::string_view title, const vec2 size) {
    const unsigned long font = utils::misc::pop_font();
...
void zgui::end_titlebox() {
    utils::misc::push_cursor_pos(window_ctx.next_cursor_pos);
    window_ctx.next_cursor_pos = {};
}

void zgui::end_groupbox2() {
    utils::misc::push_cursor_pos(window_ctx.next_cursor_pos);
    window_ctx.next_cursor_pos = {};
}
// ========================================================================
...
void zgui::key_bind(const char *id, int &value) {
    ...
    window_ctx.render.emplace_back(zgui_control_render_t{{draw_pos.x + 4, draw_pos.y + 1}, zgui_render_type::zgui_text, global_colors.color_text, active ? "press any key" : keys_list[value].data(), vec2{0, 0}, font});
...
void zgui::slider_int(const char *id, const int min, const int max, int &value) {
    ...
    const int control_width = 180; // 100
    const int control_height = 15; // 11

    const vec2 cursor_pos = utils::misc::pop_cursor_pos();
    vec2 draw_pos{window_ctx.position.x + cursor_pos.x, window_ctx.position.y + cursor_pos.y  };

    window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x + control_width - 70 /*+ 4, not -4*/, draw_pos.y - 1}, zgui_render_type::zgui_text, global_colors.color_text, id_split[0], vec2{0, 0}, font});
    ...
    int text_x = /* dynamic_width -*/ text_width -5;

  //  if (text_x < 3)
  //     text_x = 3;

    window_ctx.render.emplace_back(zgui_control_render_t{{draw_pos.x + 1, draw_pos.y + 1}, zgui_render_type::zgui_text, global_colors.color_text, value_str, vec2{0, 0}, font});
...
void zgui::slider_float(const char* id, const float min, const float max, float& value, const int decimales) {
    ...
    window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x + control_width - 70, draw_pos.y - 1 }, zgui_render_type::zgui_text, global_colors.color_text, id_split[0], vec2{0, 0}, font });
    ...
    //if (text_x < 3) // TEXT ALIGN TO LEFT!!
    //   text_x = 3;

    window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x /* + text_x*/, draw_pos.y + 1}, zgui_render_type::zgui_text, global_colors.color_text, value_str, vec2{0, 0}, font}); // MAIN TEXT

    if (utils::input::mouse_in_region(draw_pos.x, draw_pos.y, control_width, control_height)) {
        ...
        float dynamic_width_undef = std::clamp((float(value_mapped) - min) / (max - min) * control_width, 0.f, float(control_width) - 2.f);
/*
        if (dynamic_width_undef < dynamic_width) {
            alpha = 255;
            slider_a = 90;
        }
        else {
        ...
        window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x + 1, draw_pos.y + 1}, zgui_render_type::zgui_filled_rect, { 36, 61, 36, alpha }, "", {static_cast<float>(dynamic_width_undef), control_height - 2} });
    }

    window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x + 1, draw_pos.y + 1}, zgui_render_type::zgui_filled_rect, { 36, 61, 36, slider_a }, "", {static_cast<float>(dynamic_width), control_height - 2} });
    window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x + 1, draw_pos.y + 1}, zgui_render_type::zgui_filled_rect, global_colors.control_idle, "", {control_width - 2, control_height - 2} });
    window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x, draw_pos.y}, zgui_render_type::zgui_filled_rect, global_colors.control_outline, "", {control_width, control_height} });
...
bool zgui::begin_window(std::string_view title, const vec2 default_size, const unsigned long font, const int flags) {
    ...
    if (!variables::menu::yesmenukey) {
        if (!(flags & zgui_window_flags_always_open)) {
            if (utils::input::key_pressed(global_config.menu_toggle_key) || utils::input::key_pressed(global_config.menu_toggle_key2))
                window_ctx.opened = !window_ctx.opened;
        }
        else
            window_ctx.opened = true;
    }
    else if (variables::menu::yesmenukey) {
        if (!(flags & zgui_window_flags_always_open)) {
            if (utils::input::key_pressed(variables::menu::menukey))
                window_ctx.opened = !window_ctx.opened;
        }
        else
            window_ctx.opened = true;
    ...
    if (const int prev_alpha = window_ctx.alpha; !(flags & zgui_window_flags_no_ontoggle_animation)) {
        ...
        if (window_ctx.alpha != prev_alpha) {
         //   global_colors.window_border_fill.a = window_ctx.alpha;
            ...
            global_colors.color_groupbox_outline.a = window_ctx.alpha;
        }
    }

    if (window_ctx.opened || window_ctx.alpha > 0) {
        if (!(flags & zgui_window_flags_no_move)) {
            if (utils::input::mouse_in_region(window_ctx.position.x, window_ctx.position.y, window_ctx.size.x, 13)
    ...
    //    functions.draw_filled_rect(window_ctx.position.x + 1, window_ctx.position.y + 1, window_ctx.size.x - 2, window_ctx.size.y - 2, global_colors.window_border_fill);
        ...
        functions.draw_text(window_ctx.position.x + window_ctx.size.x * 0.5, window_ctx.position.y + 2, global_colors.color_text, font, true, title.data());
